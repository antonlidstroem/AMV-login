  ...
  >
    <div class="top-controls d-flex justify-content-center align-items-center gap-2 p-3 w-100">
      ...
      <div class="action-dropdown position-relative d-md-none flex-grow-1 ms-2">
        <button @click="toggleActionMenu" class="btn-secondary-custom btn-transparent" type="button"><i class="bi bi-list fs-1"></i></button>
      ...
      <div class="language-selector position-relative">
        <button @click="toggleLanguage" class="btn-secondary-custom d-flex align-items-center gap-2 w-100" type="button">
        ...
        <div class="dropdown-menu-custom w-100" :class="{ show: showLanguageMenu }">
          <button v-for="(name, code) in languageNames" :key="code" @click="selectLanguage(code as Lang)" :class="{ active: state.currentLang === code }" class="w-100" type="button">
    ...
    <ContactModal v-if="showContact" @close="handleContactClose" @show-popup="$emit('show-popup', $event)" />
    <HelpModal v-if="showHelp" @close="showHelp = false" />
    <PasswordDemands v-if="showPasswordDemands" @close="handleClosePasswordDemands" @show-popup="$emit('show-popup', $event)" />
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    watch(() => props.forceOpenContact, (newVal) => {
      if (newVal) { showContact.value = true; showHelp.value = false; showPasswordDemands.value = false; emit('contact-opened') }
    })
    ...
    // Fix: close any open dropdown when the user clicks anywhere outside it.
    // Without this, the menus stay open indefinitely after the user clicks away.
    ...
    return {
      state, languageNames, t, flagClasses, shortLanguageNames,
      showContact, showHelp, showLanguageMenu, showActionMenu, showPasswordDemands,
      toggleContact, toggleHelp, toggleLanguage, toggleActionMenu, selectLanguage, bgImage,
      handleMobileContact, handleMobileHelp, handleContactClose, handleClosePasswordDemands,
      shouldHideMobileContent
    }
  }
})
</script>
