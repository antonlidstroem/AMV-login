  ...
  style="background-color: rgba(100,100,100,0.85); width: 95%; max-width: 900px; margin: 0 auto; display: flex; flex-direction: column;">
    ...
    <form @submit.prevent="sendContact" class="d-flex flex-column gap-3 flex-grow-1">
      ...
      <div class="text-center mt-auto pt-3">
        <SecondaryButton :label="t('closeWindow')" icon="bi bi-x-circle-fill" class="btn-modal mx-auto" @action="ui.closeOverlays()" />
      </div>
    </form>
  </div>
</template>
...
const sendContact = async () => {
  // 1. Visa spinner via store istället för emit
  popup.show({
...
/* Säkerställer att textareas och inputs ser bra ut mot den mörka bakgrunden */
